...
>
    {{-- Mobile top bar --}}
    <header class="fixed left-0 right-0 top-0 z-50 border-b border-brand-950/50 bg-gradient-to-r from-brand-950 via-brand-900 to-canvas-950 pt-[env(safe-area-inset-top,0px)] shadow-lg shadow-brand-950/40 backdrop-blur-md lg:hidden">
        <div class="flex h-14 items-center gap-2 px-3">
            ...
            <a href="{{ route('dashboard') }}" class="flex min-w-0 flex-1 items-center justify-center gap-2.5">
                ...
                <span class="truncate font-display text-base font-semibold tracking-wide text-accent-50">{{ config('app.name') }}</span>
            </a>
    ...
    >
        <div class="flex flex-1 flex-col overflow-y-auto overscroll-contain p-5 pt-6 lg:p-6">
            <a href="{{ route('dashboard') }}" class="mb-8 -m-2 hidden rounded-xl p-2 transition hover:bg-white/5 lg:flex" @click="sidebarOpen = false">
                <x-boa-theme::brand size="md" :show-tagline="true" class="text-accent-100" />
            </a>

            <div class="mb-6 border-b border-brand-800/80 pb-4">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-accent-300 to-accent-600 text-sm font-bold text-brand-950 shadow-md ring-2 ring-accent-200/40">
        ...
        <div class="border-t border-brand-800/80 p-4 lg:p-6" style="padding-bottom: max(1rem, env(safe-area-inset-bottom, 0px));">

...
<div class="mb-6 md:mb-8">
    <h1 class="font-display text-2xl font-bold tracking-tight text-brand-950 sm:text-3xl">Change password</h1>
    <p class="mt-2 text-sm text-brand-900/70 sm:text-base">Update the password you use to sign in.</p>
</div>

<form method="POST" action="{{ route('password.change') }}" class="max-w-xl space-y-4 rounded-xl border border-brand-900/10 bg-white/95 p-5 shadow-sm sm:p-6">
    @csrf
    @method('PUT')
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('dashboard') }}" class="rounded-lg border border-brand-900/15 px-4 py-2.5 text-sm font-medium text-brand-900 hover:bg-brand-50">
            Cancel
        </a>
    </div>
</form>
